...
import {
  getOutputs, createOutput, updateOutput, uploadFile,
} from '../../lib/supabase'
import { TOOL_LOGOS } from '../OutputsPage'
import {
  Plus, Pencil, ExternalLink, X, Upload, Search as SearchIcon,
...
// ── AddEditModal ──────────────────────────────────────────────────────────────
function AddEditModal({ initial, agentName, onSave, onClose }) {
  const [form, setForm] = useState(initial ?? { ...EMPTY, agent: agentName })
  ...
  const handleToolChange = (val) => {
    set('ai_tool', val)
    if (logoMode === 'auto') set('logo_url', guessLogoFromTool(val))
  }
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      ...
      setError('העלאה נכשלה: ' + err.message)
    }
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <div>
              ...
              <select className={inputCls} value={catSelect} onChange={e => handleCatSelect(e.target.value)}>
          ...
          <div className="rounded-xl border border-slate-200 p-4 space-y-3">
                ...
                {Object.entries(TOOL_LOGOS).map(([name, url]) => {
                  ...
                  return (
                    <button
                      ...
                      type="button"
                      title={name}
                      onClick={() => { set('logo_url', url); if (!form.ai_tool) set('ai_tool', name) }}
                    ...
                    >
                      <img src={url} alt={name} className="w-7 h-7 object-contain" onError={e => { e.target.style.display = 'none' }} />
            ...
            {logoMode === 'upload' && (
              <div>
                ...
                <button
                  ...
                  style={{ color: '#475569' }}
                >
                  {uploadLoading ? <Loader2 size={13} className="animate-spin" /> : <ImageIcon size={13} />}
            ...
            <div className="flex items-center gap-3">
              <div className="w-16 h-16 rounded-xl border border-slate-200 flex items-center justify-center overflow-hidden bg-slate-50 flex-shrink-0">
                {form.logo_url
              ...
              <div className="flex-1 space-y-1">
                ...
                <input className={inputCls} placeholder="https://..." value={form.logo_url} onChange={e => set('logo_url', e.target.value)} />
              </div>
            </div>
...
export default function AgentOutputs() {
  ...
  const load = async () => {
    ...
    // Show only this agent's outputs
    const mine = (data || []).filter(o => o.agent === agentName)
    setOutputs(mine)
    setLoading(false)
  }
  ...
  return (
    <div dir="rtl" className="p-6 max-w-5xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-black text-slate-800">התוצרים שלי</h1>
          <p className="text-sm text-slate-500 mt-0.5">{outputs.length} תוצרים שהעלית לגלריה</p>
        </div>
        <button
              ...
              {filtered.length === 0 ? (
                <tr>
                  <td colSpan={7} className="text-center py-16 text-slate-400 text-sm">
                    {outputs.length === 0 ? 'עדיין לא העלית תוצרים — לחץ על "הוסף תוצר" כדי להתחיל' : 'לא נמצאו תוצרים תואמים'}
                  </td>
                </tr>
              ) : filtered.map(o => (
